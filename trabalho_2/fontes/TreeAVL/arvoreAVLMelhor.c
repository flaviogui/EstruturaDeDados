...
Node* insert(Node* n, int v) {
    ...
    if (v < n->v) {
        ...
        return n; // Duplicates are not allowed in AVL trees
    }
...
int main(void) {
    ...
    for (n = 1000; n <= 10000; n += 500) {
        ...
        for (i = 0; i < 10000; i++) {
            ...
            /* BUSCA O ELEMENTO DE MELHOR CASO */
            int element_present = node->v; /* Melhor caso: busca na raiz */
            gettimeofday(&b, NULL);
            int achou = search(node, element_present);
            gettimeofday(&a, NULL);
